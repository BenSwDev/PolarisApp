...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // showNotification function is now in shared.js
    ...
    $('#missionForm').addEventListener('submit', function (e) {
        ...
        if (this.dataset.mode === 'add') {
            ...
            // Check for duplicate mission names
            if (project.missions.some((m, idx) => m.name.toLowerCase() === name.toLowerCase() && idx !== editMissionIndex)) {
            ...
            if (mission.type !== type) {
                if (type === 'single') {
                    if (mission.subMissions && mission.subMissions.length > 0) {
                        showNotification('Cannot change to Single Mission while sub-missions exist. Please remove them first.', 'error');
                        return;
                    }
                    mission.type = 'single';
                    mission.status = 'not started';
                    mission.subMissions = null;
                } else if (type === 'withSubmissions') {
    ...
    function updateSubMissionsCountDisplay(missionIndex) {
        ...
        const countDisplay = document.getElementById(`subMissionsCount-${missionIndex}`);
        if (countDisplay) {
            countDisplay.innerText = `You can add ${remaining} more sub-mission(s).`;
        }
    }
    ...
    function updateMissionStatus(missionIndex, newStatus) {
        ...
        if (validTransitions[mission.status].includes(newStatus)) {
            ...
            updateProjectProgress(currentProjectIndex);
    ...
    // Expose displayProjects function globally
    ...
    $('#editSubMissionForm').addEventListener('submit', function (e) {
        ...
        // Check for duplicate sub-mission names
    ...
    // Expose updateGreetingAndTime function globally
    window.updateGreetingAndTime = updateGreetingAndTime;
